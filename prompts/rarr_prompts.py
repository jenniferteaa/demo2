"""All prompts used for RARR fact-checking and revision framework."""

# Question Generation Prompts
QGEN_PROMPT = """Please analyze this statement and generate questions to verify its factual accuracy:

Statement: {claim}

Generate exactly 3 specific questions that:
1. Focus on verifying specific factual claims
2. Target details that can be fact-checked
3. Require concrete evidence to answer

Your questions:""".strip()

CONTEXTUAL_QGEN_PROMPT = """Please analyze this statement in its context and generate verification questions:

Previous context: {context}
Current statement: {claim}

Generate exactly 3 specific questions that:
1. Focus on verifying factual claims
2. Consider how this statement relates to the context
3. Target details that can be fact-checked
4. Require concrete evidence to answer

Your questions:""".strip()

# Agreement Gate Prompts
AGREEMENT_GATE_PROMPT = """Analyze the following for factual agreement:
...
Provide clear reasoning, then state:
Therefore: [agrees/disagrees]""".strip()

CONTEXTUAL_AGREEMENT_GATE_PROMPT = """Analyze the following for factual agreement:
...
Analyze if the evidence supports or contradicts the claim, considering the context.
Provide clear reasoning, then state:
Therefore: [agrees/disagrees]""".strip()

# Editor Prompts
EDITOR_PROMPT = """Review and revise the following statement based on evidence:

Statement to revise: {claim}
Question checked: {query}
Evidence found: {evidence}

Instructions:
1. Only revise facts that the evidence clearly contradicts
2. Maintain the original meaning where possible
3. Keep revisions minimal and focused

Analysis:

My fix: [revised statement]""".strip()

CONTEXTUAL_EDITOR_PROMPT = """Review and revise the following statement based on evidence and context:
...
Instructions:
1. Consider how this statement relates to previous context
2. Only revise facts that the evidence clearly contradicts
3. Maintain the original meaning where possible
4. Keep revisions minimal and focused
5. Ensure consistency with context
...
EVIDENCE_ASSESSMENT_PROMPT = """Evaluate the following evidence:

Query: {query}
Evidence: {evidence}

Assess this evidence for:
1. Relevance to the query
2. Factual support
3. Reliability

Your assessment:""".strip()

CONTEXTUAL_EVIDENCE_ASSESSMENT_PROMPT = """Evaluate the following evidence in context:
